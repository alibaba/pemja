...
static int pyjobject_init(JNIEnv *env, PyJObject *self) {
  int len;
  JcpThread *jcpThread;
  ...
  if (cachedAttrs == NULL) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (pyjfield->fd_name) {
        if (PyDict_SetItem(cachedAttrs, pyjfield->fd_name,
                           (PyObject *)pyjfield) < 0) {
          Py_DECREF(pyjfield);
          goto EXIT_ERROR;
        }
...
static PyObject *pyjobject_getattro(PyObject *self, PyObject *name) {
...
static int pyjobject_setattro(PyJObject *self, PyObject *name,
                              PyObject *value) {
  ...
  if (!PyJField_Check(attr)) {
    PyErr_Format(PyExc_AttributeError, "'%s' object cannot set attribute '%s'.",
                 PyUnicode_AsUTF8(self->class_name), PyUnicode_AsUTF8(name));
  }
...
PyObject *JcpPyJObject_New(JNIEnv *env, PyTypeObject *type, jobject object,
                           jclass clazz) {
  PyJObject *self;
...
PyTypeObject PyJObject_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "pemja.PyJObject", /* tp_name */
