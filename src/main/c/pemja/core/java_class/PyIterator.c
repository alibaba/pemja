...
jobject JavaPyIterator_New(JNIEnv *env, jlong tstate, jlong pyobject) {
  if (!init_PyIterator) {
    init_PyIterator =
        (*env)->GetMethodID(env, JPYITERPRETER_TYPE, "<init>", "(JJ)V");
  }
  return (*env)->NewObject(env, JPYITERPRETER_TYPE, init_PyIterator, tstate,
                           pyobject);
}
