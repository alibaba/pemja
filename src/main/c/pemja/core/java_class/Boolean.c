...
jobject JavaBoolean_New(JNIEnv* env, jboolean jval) {
  if (!init_Z) {
...
jboolean JavaBoolean_booleanValue(JNIEnv* env, jobject jval) {
  if (!booleanValue) {
    booleanValue =
        (*env)->GetMethodID(env, JBOOLEAN_OBJ_TYPE, "booleanValue", "()Z");
  }
